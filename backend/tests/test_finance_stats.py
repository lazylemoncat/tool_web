...
def test_stats_response_matches_frontend_chart_contract(client, auth_headers):
    ledger_id, account_id, category_id = _create_ledger_account_category(
        client, auth_headers
    )
    _create_transaction(
        client,
    ...
    _create_transaction(
        client, auth_headers, ledger_id, account_id, "income", "500.00"
    )

    resp = client.get(
        f"/api/v1/finance/stats?ledger_id={ledger_id}&period=month",
        headers=auth_headers,
    )
