...
def _build_folder_out(folder: Folder, db: Session) -> FolderOut:
    children = sorted(folder.children or [], key=lambda x: (x.sort_order, x.id))
    return FolderOut(
        id=folder.id,
...
def list_folders(
    ...
    folders = q.order_by(Folder.sort_order, Folder.id).options(selectinload(Folder.children)).offset(skip).limit(limit).all()
    return [_build_folder_out(f, db) for f in folders]


@router.post("", response_model=FolderOut, status_code=201)
def create_folder(body: FolderCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def update_folder(folder_id: int, body: FolderUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def batch_reorder(body: ReorderBatch, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ids = [item.id for item in body.items]
    folders = db.query(Folder).filter(
        Folder.id.in_(ids), Folder.user_id == current_user.id
    ).all()
