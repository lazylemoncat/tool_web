...
class AuthStore:
    ...
    def create_user(self, *, username: str, password_hash: str, is_admin: bool = False) -> User:
        ...
        self.db.add(user)
        self.db.flush()
        prefs = UserPreference(user_id=user.id, preferences_json=dict(DEFAULT_PREFERENCES))
        self.db.add(prefs)
        self.db.flush()
        return user
    ...
    def get_preferences(self, user_id: int) -> dict[str, Any]:
        ...
        if not prefs:
            prefs = UserPreference(user_id=user_id, preferences_json=dict(DEFAULT_PREFERENCES))
            self.db.add(prefs)
            self.db.flush()
    ...
    def update_preferences(self, user_id: int, preferences: dict[str, Any]) -> None:
        prefs = self.db.query(UserPreference).filter(UserPreference.user_id == user_id).first()
        if not prefs:
            prefs = UserPreference(user_id=user_id, preferences_json=preferences)
            self.db.add(prefs)
        else:
            prefs.preferences_json = preferences
            prefs.updated_at = utcnow()

    def create_session(self, *, user_id: int, ttl_seconds: int, ip: str | None, user_agent: str | None) -> AuthSession:
        session = AuthSession(
            id=make_id(),
            user_id=user_id,
            ip_address=ip,
    ...
    def revoke_user_sessions(self, user_id: int, reason: str, except_session_id: str | None = None) -> None:
        q = self.db.query(AuthSession).filter(AuthSession.user_id == user_id, AuthSession.revoked_at == None)
    ...
    def create_refresh_token(self, *, session_id: str, ttl_seconds: int, token_family: str | None = None) -> tuple[str, AuthRefreshToken]:
    ...
    def get_refresh_by_plaintext(self, token: str) -> AuthRefreshToken | None:
        return self.db.query(AuthRefreshToken).filter(AuthRefreshToken.token_hash == hash_token(token)).first()

    def mark_refresh_used(self, refresh: AuthRefreshToken, replaced_by_id: str) -> None:
        refresh.used_at = utcnow()
        refresh.replaced_by_id = replaced_by_id

    def revoke_refresh_family(self, token_family: str) -> None:
        for refresh in self.db.query(AuthRefreshToken).filter(AuthRefreshToken.token_family == token_family).all():
    ...
    def enabled_mfa_methods(self, user_id: int) -> list[AuthMfaMethod]:
        return (
            ...
            .filter(AuthMfaMethod.user_id == user_id, AuthMfaMethod.is_enabled == True)
            .all()
        )

    def create_totp_method(self, *, user_id: int, secret_ciphertext: str) -> AuthMfaMethod:
    ...
    def get_mfa_method(self, method_id: str, user_id: int) -> AuthMfaMethod | None:
        return (
            self.db.query(AuthMfaMethod)
            .filter(AuthMfaMethod.id == method_id, AuthMfaMethod.user_id == user_id)
            .first()
        )
    ...
    def get_mfa_challenge(self, challenge_id: str) -> AuthMfaChallenge | None:
        return self.db.query(AuthMfaChallenge).filter(AuthMfaChallenge.id == challenge_id).first()

    def is_totp_step_used(self, *, user_id: int, method_id: str, time_step: int) -> bool:
        return (
    ...
    def mark_totp_step_used(self, *, user_id: int, method_id: str, time_step: int, challenge_id: str | None = None) -> None:
        self.db.add(
            AuthMfaUsedTotpStep(
                user_id=user_id,
                method_id=method_id,
    ...
    def create_recovery_codes(self, *, user_id: int, codes: list[str]) -> None:
        for old in self.db.query(AuthRecoveryCode).filter(AuthRecoveryCode.user_id == user_id, AuthRecoveryCode.revoked_at == None).all():
    ...
    def consume_recovery_code(self, *, user_id: int, code: str, ip: str | None, user_agent: str | None) -> bool:
        recovery = (
            self.db.query(AuthRecoveryCode)
            .filter(
                AuthRecoveryCode.user_id == user_id,
                AuthRecoveryCode.code_hash == hash_token(code),
                AuthRecoveryCode.used_at == None,
                AuthRecoveryCode.revoked_at == None,
            )
            .first()
        )
        if not recovery:
...
def session_is_active(session: AuthSession) -> bool:
    ...
    return session.revoked_at is None and expires_at is not None and expires_at > utcnow()


def refresh_is_active(refresh: AuthRefreshToken) -> bool:
    expires_at = ensure_aware_utc(refresh.expires_at)
    return refresh.revoked_at is None and expires_at is not None and expires_at > utcnow()
