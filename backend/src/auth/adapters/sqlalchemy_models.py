...
class AuthRefreshToken(Base):
    ...
    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    session_id: Mapped[str] = mapped_column(String(64), ForeignKey("auth_sessions.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    revoked_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    replaced_by_id: Mapped[str | None] = mapped_column(String(64), ForeignKey("auth_refresh_tokens.id"), nullable=True)
...
class AuthAuditLog(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    user_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("auth_users.id", ondelete="SET NULL"), nullable=True, index=True)
    session_id: Mapped[str | None] = mapped_column(String(64), ForeignKey("auth_sessions.id", ondelete="SET NULL"), nullable=True, index=True)
...
class AuthMfaChallenge(Base):
    ...
    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey("auth_users.id", ondelete="CASCADE"), nullable=False, index=True)
    session_intent_id: Mapped[str | None] = mapped_column(String(64), nullable=True)
    challenge_type: Mapped[str] = mapped_column(String(50), nullable=False)
    available_methods_json: Mapped[list[str]] = mapped_column(JSON, nullable=False)
    ip_address: Mapped[str | None] = mapped_column(String(100), nullable=True)
    user_agent: Mapped[str | None] = mapped_column(String(500), nullable=True)
...
class AuthRecoveryCode(Base):
    ...
    used_ip_address: Mapped[str | None] = mapped_column(String(100), nullable=True)
    used_user_agent: Mapped[str | None] = mapped_column(String(500), nullable=True)
    revoked_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class AuthMfaUsedTotpStep(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey("auth_users.id", ondelete="CASCADE"), nullable=False, index=True)
    method_id: Mapped[str] = mapped_column(String(64), ForeignKey("auth_mfa_methods.id", ondelete="CASCADE"), nullable=False)
    ...
    challenge_id: Mapped[str | None] = mapped_column(String(64), ForeignKey("auth_mfa_challenges.id"), nullable=True)
