"""
Finance data models: Ledger, Account, Category, Tag, Transaction, SplitItem,
Event, Budget, Attachment, ResourceRelation.
"""
...
class Ledger(Base):
    ...
    icon: Mapped[str] = mapped_column(
        String(10), default="\U0001f4b0", nullable=True
    )
    ...
    accounts: Mapped[list[Account]] = relationship(
        "Account", back_populates="ledger", cascade="all, delete-orphan"
    )
    ...
    tags: Mapped[list[FinanceTag]] = relationship(
        "FinanceTag", back_populates="ledger", cascade="all, delete-orphan"
    )
    transactions: Mapped[list[Transaction]] = relationship(
        "Transaction", back_populates="ledger", cascade="all, delete-orphan"
    )
    events: Mapped[list[Event]] = relationship(
        "Event", back_populates="ledger", cascade="all, delete-orphan"
    )
    budgets: Mapped[list[Budget]] = relationship(
        "Budget", back_populates="ledger", cascade="all, delete-orphan"
    )
...
class FinanceCategory(Base):
    ...
    ledger: Mapped[Ledger] = relationship(
        "Ledger", back_populates="categories"
    )
    parent: Mapped[FinanceCategory | None] = relationship(
        "FinanceCategory", remote_side=[id], back_populates="children"
    )
    ...
    split_items: Mapped[list[SplitItem]] = relationship(
        "SplitItem", back_populates="category"
    )
...
class Transaction(Base):
    ...
    __table_args__ = (
        Index(
            "idx_tx_user_ledger_date", "user_id", "ledger_id", "occurred_at"
        ),
    ...
    user: Mapped[User] = relationship("User")
    ledger: Mapped[Ledger] = relationship(
        "Ledger", back_populates="transactions"
    )
    account: Mapped[Account] = relationship(
        "Account", back_populates="transactions"
    )
    category: Mapped[FinanceCategory | None] = relationship(
    ...
    parent: Mapped[Transaction | None] = relationship(
        "Transaction", remote_side=[id], back_populates="children"
    )
    children: Mapped[list[Transaction]] = relationship(
        "Transaction", back_populates="parent", cascade="all, delete-orphan"
    )
    tags: Mapped[list[FinanceTag]] = relationship(
        "FinanceTag", secondary=transaction_tags, back_populates="transactions"
    )
    split_items: Mapped[list[SplitItem]] = relationship(
        "SplitItem", back_populates="transaction", cascade="all, delete-orphan"
    )
    attachments: Mapped[list[Attachment]] = relationship(
        "Attachment",
        secondary=transaction_attachments,
...
class SplitItem(Base):
    ...
    transaction: Mapped[Transaction] = relationship(
        "Transaction", back_populates="split_items"
    )
    category: Mapped[FinanceCategory | None] = relationship(
        "FinanceCategory", back_populates="split_items"
    )


class Event(Base):
    __tablename__ = "events"
    __table_args__ = (Index("idx_events_user_ledger", "user_id", "ledger_id"),)
    ...
    relations_as_from: Mapped[list[ResourceRelation]] = relationship(
        ...
        primaryjoin=(
            "and_(Event.id==foreign(ResourceRelation.from_id), "
            "ResourceRelation.from_type=='event')"
        ),
        cascade="all, delete-orphan",
    )
    relations_as_to: Mapped[list[ResourceRelation]] = relationship(
        ...
        primaryjoin=(
            "and_(Event.id==foreign(ResourceRelation.to_id), "
            "ResourceRelation.to_type=='event')"
        ),
        cascade="all, delete-orphan",
    )
