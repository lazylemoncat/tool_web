/*
 BudgetForm — 创建/编辑预算. 使用新 Modal 包装.
*/

import React, { useState } from 'react'
import { useLocale } from '../../i18n'
import { Modal, FormField, NumberInput, FormFooter } from '../ui'
...
const BudgetForm: React.FC<Props> = ({ editBudget, categories, tags, events, onSubmit, onClose }) => {
  ...
  const [selectedCategories, setSelectedCategories] = useState<number[]>(editBudget?.filters?.category_ids ?? [])
  const [selectedTags, setSelectedTags] = useState<number[]>(editBudget?.filters?.tag_ids ?? [])
  const [selectedEvents, setSelectedEvents] = useState<number[]>(editBudget?.filters?.event_ids ?? [])
  ...
  const handleSubmit = () => {
    ...
    if (!valid) return
    onSubmit({
      name: name.trim(), amount: numAmount, currency: currency || 'CNY',
      rrule: rrule.trim() || null, filters: buildFilters(), rollover, alert_threshold: alertThreshold,
    })
  }

  return (
    <Modal open onOpenChange={() => onClose()} size="lg" title={isEdit ? t('finance.editBudget') : t('finance.newBudget')} footer={<FormFooter onCancel={onClose} onSubmit={handleSubmit} />}>
      ...
      <div className="form-row">
        <div style={{ flex: 1 }}>
          <FormField label={t('finance.budgetAmount')} error={amountError} required>
            <NumberInput value={amount ? parseFloat(amount) : null} onChange={(v) => { setAmount(v !== null ? String(v) : ''); if (amountError) setAmountError('') }} decimals={2} min={0} placeholder="0.00" />
          </FormField>
        </div>
        <div style={{ flex: 1 }}>
          <FormField label={`${t('finance.alertThreshold')} (%)`}>
            <input type="number" value={alertThreshold} onChange={(e) => setAlertThreshold(Math.min(100, Math.max(1, Number(e.target.value) || 80)))} min="1" max="100" />
          </FormField>
        </div>
      </div>
        ...
        <div className="finance-checkbox-group">
          {allCats.map((cat) => (
            <label key={cat.id} className="finance-checkbox-label">
              <input type="checkbox" checked={selectedCategories.includes(cat.id)} onChange={() => toggleItem(cat.id, selectedCategories, setSelectedCategories)} />
              {cat.name}
            </label>
          ))}
        </div>
      ...
      <div className="form-group">
        ...
        <div className="finance-tag-chips">
          {tags.map((tag) => (
            <button key={tag.id} className={`finance-tag-chip ${selectedTags.includes(tag.id) ? 'active' : ''}`} onClick={() => toggleItem(tag.id, selectedTags, setSelectedTags)} type="button">
              {tag.name}
            </button>
          ))}
        </div>
        ...
        <div className="finance-checkbox-group">
          {events.map((ev) => (
            <label key={ev.id} className="finance-checkbox-label">
              <input type="checkbox" checked={selectedEvents.includes(ev.id)} onChange={() => toggleItem(ev.id, selectedEvents, setSelectedEvents)} />
