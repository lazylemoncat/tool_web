    ...
    <Box sx={{ height: '100%', overflowY: 'auto', bgcolor: '#F5F6FA', px: { xs: 2, sm: 3 }, py: 3 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 2 }}>
        <Box>
          ...
          <Typography sx={{ fontSize: '0.8125rem', color: '#4B5563' }}>日常账本 · 共 {total} 笔交易</Typography>
        </Box>
        <Button variant="contained" size="small" onClick={onNewTransaction}
          sx={{ borderRadius: 2.5, px: 2.75, py: 1, fontSize: '0.8125rem', fontWeight: 600, textTransform: 'none', boxShadow: 'none', bgcolor: '#6D5DFC', height: 40, '&:hover': { bgcolor: '#5A4DE0' } }}>
          + 记一笔
        </Button>
      </Box>
      ...
      {/* Transaction List */}
      {filtered.length === 0 ? (
        <Box sx={{ bgcolor: 'background.paper', borderRadius: 3, p: 8, textAlign: 'center', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', border: '1px solid', borderColor: '#EDECF0' }}>
        ...
        <Box sx={{ bgcolor: 'background.paper', borderRadius: 3, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', border: '1px solid', borderColor: '#EDECF0' }}>
          {filtered.map((tx, i) => (
